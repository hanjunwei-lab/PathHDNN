...
def evaluate(model, graph, dataloader):
    ...
    for it, (input_nodes, output_nodes, blocks) in enumerate(dataloader):
        with torch.no_grad():
            ...
            y.append(blocks[-1].dstdata["label"])  # 假设 y 是独热编码
    ...
    # 将 y 转换为类标签
    if y.dim() > 1:  # 检查 y 是否是独热编码
        y = y.argmax(dim=1)  # 转换为类标签

    # 计算准确率
    acc += (logits.argmax(dim=1) == y).float().mean().item()  # 直接比较类标签
    
    # 计算损失
    loss = loss_fcn(logits, y)  # 计算损失

    # 生成概率分布
    probabilities = nn.functional.softmax(logits, dim=1).cpu().detach().numpy()
    
    # 生成二进制输出，确保是一个二维数组
    binary_out = (probabilities >= 0.5).astype(int)  # 使用阈值将概率转为二进制

    # 获取真实标签
    labels_out = y.cpu().detach().numpy()

    # 确保 labels_out 是一维数组
    if labels_out.ndim > 1:
        labels_out = labels_out.argmax(axis=1)  # 转换为类标签
    PRC = average_precision_score(labels_out, probabilities[:, 1], average="weighted")  # 只使用正类的概率
    SNS = recall_score(labels_out, binary_out.argmax(axis=1), average="weighted")
    # 计算评估指标（多分类的时候）
    #PRC = average_precision_score(labels_out, probabilities, average="weighted")  # 修改为正确的参数顺序
    #SNS = recall_score(labels_out, binary_out.argmax(axis=1), average="weighted")
    F1 = 2 * ((PRC * SNS) / (PRC + SNS))

    return loss.item(), acc, F1, PRC, SNS, logits, y
