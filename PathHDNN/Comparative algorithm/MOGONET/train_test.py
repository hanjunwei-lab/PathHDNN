...
def prepare_trte_data(data_folder, view_list):
    ...
    with open(os.path.join(data_folder, "labels_tr.csv"), 'r', encoding='utf-8-sig') as f:
        labels_tr = np.loadtxt(f, delimiter=',')  # 读取数据
    with open(os.path.join(data_folder, "labels_te.csv"), 'r', encoding='utf-8-sig') as f:
        labels_te = np.loadtxt(f, delimiter=',')  # 读取数据
...
def train_test(data_folder, view_list, num_class,
    ...
    for epoch in range(num_epoch+1):
        ...
        if epoch % test_inverval == 0:
            ...
            print()
        # 保存模型
    #model_save_path = os.path.join(data_folder, "trained_model.model")
    torch.save(model_dict, "data(li)delete10//训练//trained_model.model")
    #print(f"模型已保存到 {model_save_path}")
    final_predictions = te_prob.argmax(1).astype(int)  # 获取最终预测类别并转换为整数
    predictions_save_path = os.path.join("data(li)delete10", "final_predictions.csv")
    np.savetxt(predictions_save_path, final_predictions, delimiter=",", header="Predictions", comments="", fmt='%d')  # 使用'%d'格式保存为整数
    print(f"最终预测结果已保存到 {predictions_save_path}")
